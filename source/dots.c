...
void DOT_savegame(FILE *h) {
  ...
  for (i = 0; i < MAXDOT; ++i)
    if (dot[i].t)
      myfwrite(&dot[i], 1, sizeof(dot_t), h);
}
...
void DOT_draw(void) {
  ...
  for (i = 0; i < MAXDOT; ++i)
    if (dot[i].t)
      V_dot(
          dot[i].o.x - w_x + WD / 2, dot[i].o.y - w_y + HT / 2 + 1 + w_o,
          dot[i]
              .c); // if(dot[i].t)
                   // V_dot(dot[i].o.x-w_x+100,dot[i].o.y-w_y+50+w_o,dot[i].c);
}
