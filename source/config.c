...
static cfg_t cfg[] = {{"file", NULL, NULL, FILES, 0},
...
void CFG_args(int argc, char *argv[]) {
  ...
  logo("CFG_args: parsing command line args\n");
      ...
      if (cfg[j].par)
        if (strcasecmp(s, cfg[j].par) == 0) {
          ...
          case FILES:
            ...
            break;
          default:
            ERR_failinit("illegal CFG string");
          }
          cfg[j].o = 1;
          break;
        }
...
void CFG_load(void) {
  ...
  logo("CFG_load: loading cfg file %s\n", pc);
          ...
          case KEY: {
            ...
            if (k) {
              *((int *)cfg[j].p) = k;
              ...
              logo("unknown key name in cfg: %s=%s\n", p1, p2);
              logo("available key names:\n");
              int i;
              for (i = 0; i < 32; i++) {
                char *s = key_names[i];
