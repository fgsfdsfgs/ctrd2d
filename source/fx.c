...
static int isin(int a) { return sintab[a & SINPM]; }
...
void FX_draw(void) {
  ...
  for (i = 0; i < MAXFX; ++i) {
    ...
    case BUBL:
      V_dot(
          (fx[i].x >> 8) - w_x + WD / 2,
          (fx[i].y >> 8) - w_y + HT / 2 + 1 + w_o,
          0xC0 +
              fx[i]
                  .s); // V_dot((fx[i].x>>8)-w_x+100,(fx[i].y>>8)-w_y+50+w_o,0xC0+fx[i].s);
      continue;
    }
    if (s >= 0)
...
void FX_bubble(int x, int y, int xv, int yv, int n) {
  ...
  for (; n > 0; --n) {
    ...
    fx[i].t = BUBL;
    fx[i].s = rand() & 3;
    fx[i].x = (x << 8) + myrand(513) - 256;
    fx[i].y = (y << 8) + myrand(513) - 256;
    fx[i].xv = xv;
    fx[i].yv = yv - myrand(256) - 768;
  }
}
