...
typedef struct { int r, h, l, mp, rv, jv, sp, minp; } mnsz_t;

byte nomon = 1;

static char *sleepanim[MN_TN] = {"AAABBB", "AAABBB", "AAABBB", "AAABBB",
                                 "AAABBB", "AAABBB", "AAABBB", "AAABBB",
                                 "A",      "AAABBB", "AAABBB", "AAABBB",
                                 "AAABBB", "AAABBB", "AAABBB", "AAABBB",
                                 "AAABBB", "A",      "A",      "AAABBB"},
            *goanim[MN_TN] = {"AABBCCDD",
                              "AABBCCDD",
                              "AABBDDAACCDD",
                              "AABBDDAACCDD",
                              "AABBDDCCDDBB",
                              ...
                              "AABB",
                              "A",
                              "DDEEFFGGHHIIJJKKLLAABBCC",
                              "ACDABD"},
            *painanim[MN_TN] = {"H", "H", "G",        "G", "G", "G", "H",
                                "H", "F", "E",        "G", "I", "I", "J",
                                "L", "Q", "EECCDDCC", "A", "D", "G"},
            *waitanim[MN_TN] = {"A", "A", "A",    "A", "A", "A", "A",
                                "A", "A", "AABB", "A", "A", "A", "I",
                                "K", "A", "A",    "A", "D", "E"},
            *attackanim[MN_TN] =
                {"EEFFGG", "EEFFGG", "EEEEEF", "EEEEEF",
                 "EEEEEF", "EF",     "EEFFGG", "EEFFGG",
                 "BBCCDD", "CCDD",   "DDEEFF", "GH",
                 "GH",     "GGGGHH", "GGHHII", "QQGGGHHHIIJJKKLLMMNNOOPP",
                 "BBFFAA", "A",      "OOPPQQ", "EEEEFF"},
            *dieanim[MN_TN] = {"IIIJJJKKKLLLMMM",
                               "IIIJJJKKKLLL",
                               "HHHIIIJJJKKK",
                               "HHHIIIJJJKKK",
                               ...
                               "FFFGGGHHHIIIJJJKKK",
                               "HHHIIIJJJKKKLLLMMM",
                               "JJJJKKKKLLLLMMMMNNNNOOOOPPPPQQQQRRRR",
                               "JJJKKKLLLMMMNNNOOO",
                               "KKKLLLMMMNNNOOOPPPRRRSSS",
                               "MMMNNNOOOPPP",
                               "RRRSSSTTTUUUVVVWWWXXXYYY",
                               "DDDD",
                               "CCCDDDEEEFFFGGG",
                               "D",
            ...
            *slopanim[MN_TN] = {"",
                                ...
                                "",
                                "OOPPQQRRSSTTUUVV"},
            *deadanim[MN_TN] = {"N", "M", "L", "L", "P", "N", "O",
                                "O", "L", "",  "",  "S", "P", "T",
                                "Q", "Z", "C", "",  "D", "N"},
            *messanim[MN_TN] = {"", "U", "U", "U", "", "T", "", "", "", "",
                                "", "",  "",  "",  "", "",  "", "", "", "W"};
...
static void *snd[MN_TN][5], *impsitsnd[2], *impdthsnd[2], *firsnd, *slopsnd,
    *gsnd[4];
...
void MN_alloc(void) {
  ...
  static char sn[MN_TN][5][6] =
      {{"DMACT", "DMPAIN", "SGTATK", "SGTSIT", "SGTDTH"},
       ...
       {"HAHA1", "PLPAIN", "", "STOP1", "PDIEHI"}},
              msn[MN_TN][4] = {"SARG", "TROO", "POSS", "SPOS", "CYBR",
                               "CPOS", "BOSS", "BOS2", "HEAD", "SKUL",
                               "PAIN", "SPID", "BSPI", "FATT", "SKEL",
                               "VILE", "FISH", "BAR1", "ROBO", "PLAY"};
  static char gsn[6] = "GOOD0";
  static int mms[MN_TN] = {14 * 2, 21 * 2, 21 * 2, 21 * 2, 16 * 2,
                           20 * 2, 15 * 2, 15 * 2, 12 * 2, 11 * 2,
                           13 * 2, 19 * 2, 16 * 2, 20 * 2, 17 * 2,
                           29 * 2, 6 * 2,  2 * 2,  17 * 2, 23 * 2};
  ...
  for (j = 0; j < MN_TN; ++j) {
    ...
    if (j == MN_BARREL - 1)
      for (i = 4; i < 14; ++i)
        spr[j][i] = Z_getspr("BEXP", i / 2 - 2, (i & 1) + 1, &sprd[j][i]);
    for (i = 0; i < 5; ++i)
      if (sn[j][i][0])
...
static void setst(int i, int st) {
  ...
  case ATTACK:
    a = attackanim[t];
    if (st == ATTACK && t == MN_VILE - 1)
      a = "[[\\\\]]";
    break;
  case DIE:
    if (g_map == 9 && t == MN_BSP - 1)
      Z_sound(pauksnd, 128);
    a = dieanim[t];
    break;
  case DEAD:
    a = deadanim[t];
    if (mn[i].ap == slopanim[t])
      a = messanim[t];
    if (t == MN_BARREL - 1) {
      mn[i].t = 0;
    }
...
int MN_spawn_deadpl(obj_t *o, byte c, int t) {
  ...
  if ((i = MN_spawn(o->x, o->y, c, t + MN_PL_DEAD)) == -1)
    return -1;
  mn[i].o = *o;
  return i;
}
...
static int isfriend(int a, int b) {
  ...
  if (a == MN_SOUL && b == MN_PAIN)
    return 1;
  if (b == MN_SOUL && a == MN_PAIN)
    return 1;
  return 0;
}
...
static int MN_findnewprey(int i) {
  ...
  if (a) {
    if (b)
      mn[i].aim = (abs(mn[i].o.x - pl1.o.x) + abs(mn[i].o.y - pl1.o.y) <=
                   abs(mn[i].o.x - pl2.o.x) + abs(mn[i].o.y - pl2.o.y))
                      ? -1
                      : -2;
    else
      mn[i].aim = -1;
    ...
    if (b)
      ...
      for (a = 0, b = 32000, mn[i].aim = -3; a < MAXMN; ++a)
        if (mn[a].t && mn[a].st != DEAD && a != i &&
            !isfriend(mn[a].t, mn[i].t))
          if ((l = abs(mn[i].o.x - mn[a].o.x) + abs(mn[i].o.y - mn[a].o.y)) <
              b) {
            mn[i].aim = a;
            b = l;
          }
...
int Z_getobjpos(int i, obj_t *o) {
  if (i == -1) {
    *o = pl1.o;
    return !PL_isdead(&pl1);
  }
  if (_2pl)
    if (i == -2) {
      *o = pl2.o;
      return !PL_isdead(&pl2);
    }
  if (i >= 0 && i < MAXMN)
    if (mn[i].t && mn[i].st != DEAD) {
      *o = mn[i].o;
      return 1;
    }
  return 0;
...
static int shoot(int i, obj_t *o, int n) {
  ...
  if (!n)
    ...
    case MN_ROBO:
      if (++mn[i].ammo >= 50)
        mn[i].ammo = (mn[i].t == MN_ROBO) ? -200 : -50;
      break;
    case MN_MAN:
      break;
    case MN_MANCUB:
  ...
  if (!Z_look(&mn[i].o, o, mn[i].d))
    return 0;
  mn[i].atm = 0;
  mn[i].tx = o->x + (o->xv + o->vx) * 6;
  mn[i].ty = o->y - o->h / 2 + (o->yv + o->vy) * 6;
  if (abs(mn[i].tx - mn[i].o.x) < abs(mn[i].ty - mn[i].o.y + mn[i].o.h / 2))
    return 0;
  switch (mn[i].t) {
  case MN_IMP:
  case MN_BARON:
...
void MN_act(void) {
  ...
  if (abs(pt_x += pt_xs) > 123)
    pt_xs = -pt_xs;
  if (abs(pt_y += pt_ys) > 50)
    pt_ys = -pt_ys;
  if (gsndt > 0)
    if (--gsndt == 0) {
  ...
  for (i = 0; i < MAXMN; ++i)
    if ((t = mn[i].t) != 0) {
      switch (t) {
      case MN_FISH:
        if (!Z_inwater(mn[i].o.x, mn[i].o.y, mn[i].o.r, mn[i].o.h))
          break;
      case MN_SOUL:
      case MN_PAIN:
      case MN_CACO:
        if (mn[i].st != DIE && mn[i].st != DEAD)
          --mn[i].o.yv;
        break;
      }
      z_mon = 1;
      ...
      if (st & Z_HITWATER)
        Z_splash(&mn[i].o, mn[i].o.r + mn[i].o.h);
      SW_press(mn[i].o.x, mn[i].o.y, mn[i].o.r, mn[i].o.h, 8, i);
      if (mn[i].ftime) {
        --mn[i].ftime;
        SMK_flame(mn[i].o.x, mn[i].o.y - mn[i].o.h / 2, mn[i].o.xv + mn[i].o.vx,
                  mn[i].o.yv + mn[i].o.vy, mn[i].o.r / 2, mn[i].o.h / 2,
                  rand() % (200 * 2 + 1) - 200, -500, 1, mn[i].fobj);
      }
      if (st & Z_INWATER)
        mn[i].ftime = 0;
      if (mn[i].st == DEAD)
      ...
      if (t == MN_BARREL) {
        ...
        } else if (mn[i].st == DIE && mn[i].ac == 2)
          Z_explode(mn[i].o.x, mn[i].o.y - 8, 30, mn[i].aim);
        continue;
      }
      if (t == MN_SOUL)
        if (st & Z_HITAIR)
          Z_set_speed(&mn[i].o, 16);
      if (mn[i].ammo < 0)
        ++mn[i].ammo;
      if (mn[i].o.yv < 0)
        if (st & Z_INWATER)
      ...
      case RUN:
        ...
        if (--mn[i].s <= 0 ||
            ((st & Z_HITWALL) && mn[i].o.yv + mn[i].o.vy == 0)) {
          setst(i, GO);
          mn[i].s = 0;
      ...
      case SHOOT:
        ...
        if (t != MN_FISH)
          mn[i].o.xv = Z_dec(mn[i].o.xv, 1);
        if (t == MN_VILE && mn[i].st == SHOOT) {
          if (!Z_getobjpos(mn[i].aim, &o)) {
            setst(i, GO);
            break;
          }
          if (!Z_look(&mn[i].o, &o, mn[i].d)) {
      ...
      if (!mn[i].ap[mn[i].ac])
        ...
        case ATTACK:
          ...
          if (t != MN_SOUL && mn[i].st != DIE)
            setst(i, GO);
          break;
        case SHOOT:
          switch (t) {
          case MN_IMP:
            WP_ball1(mn[i].o.x + (mn[i].d * 2 - 1) * mn[i].o.r,
                     mn[i].o.y - mn[i].o.h / 2, mn[i].tx, mn[i].ty, i);
            break;
          case MN_ZOMBY:
            WP_pistol(mn[i].o.x + (mn[i].d * 2 - 1) * mn[i].o.r,
                      mn[i].o.y - mn[i].o.h / 2, mn[i].tx, mn[i].ty, i);
            break;
          case MN_SERG:
            WP_shotgun(mn[i].o.x + (mn[i].d * 2 - 1) * mn[i].o.r,
                       mn[i].o.y - mn[i].o.h / 2, mn[i].tx, mn[i].ty, i);
            break;
          case MN_MAN:
            WP_dshotgun(mn[i].o.x + (mn[i].d * 2 - 1) * mn[i].o.r,
                        mn[i].o.y - mn[i].o.h / 2, mn[i].tx, mn[i].ty, i);
            mn[i].ammo = -36;
            break;
          case MN_CYBER:
            WP_rocket(mn[i].o.x + (mn[i].d * 2 - 1) * mn[i].o.r,
                      mn[i].o.y - mn[i].o.h / 2, mn[i].tx, mn[i].ty, i);
            break;
          case MN_SKEL:
            WP_revf(mn[i].o.x + (mn[i].d * 2 - 1) * mn[i].o.r,
                    mn[i].o.y - mn[i].o.h / 2, mn[i].tx, mn[i].ty, i,
                    mn[i].aim);
            break;
          case MN_CGUN:
          case MN_SPIDER:
            WP_mgun(mn[i].o.x + (mn[i].d * 2 - 1) * mn[i].o.r,
                    mn[i].o.y - mn[i].o.h / 2, mn[i].tx, mn[i].ty, i);
            break;
          case MN_BSP:
            WP_aplasma(mn[i].o.x + (mn[i].d * 2 - 1) * mn[i].o.r,
                       mn[i].o.y - mn[i].o.h / 2, mn[i].tx, mn[i].ty, i);
            break;
          case MN_ROBO:
            WP_plasma(mn[i].o.x + (mn[i].d * 2 - 1) * 15, mn[i].o.y - 30,
                      mn[i].tx, mn[i].ty, i);
            break;
          case MN_MANCUB:
            WP_manfire(mn[i].o.x + (mn[i].d * 2 - 1) * mn[i].o.r,
                       mn[i].o.y - mn[i].o.h / 2, mn[i].tx, mn[i].ty, i);
            break;
          case MN_BARON:
          case MN_KNIGHT:
            WP_ball7(mn[i].o.x, mn[i].o.y - mn[i].o.h / 2, mn[i].tx, mn[i].ty,
                     i);
            break;
          case MN_CACO:
            WP_ball2(mn[i].o.x, mn[i].o.y - mn[i].o.h / 2, mn[i].tx, mn[i].ty,
                     i);
            break;
          case MN_PAIN:
            if ((sx = MN_spawn(mn[i].o.x, mn[i].o.y, mn[i].d, MN_SOUL)) == -1)
              break;
          ...
          if (t == MN_CGUN || t == MN_SPIDER || t == MN_BSP || t == MN_MANCUB ||
              t == MN_ROBO)
            if (!Z_getobjpos(mn[i].aim, &o))
              MN_findnewprey(i);
            else if (shoot(i, &o, 0))
        ...
        case DIE:
          setst(i, DEAD);
          if (t == MN_PAIN || t == MN_SOUL)
            mn[i].ftime = 0;
          if (t == MN_PAIN) {
            if ((sx = MN_spawn(mn[i].o.x - 15, mn[i].o.y, 0, MN_SOUL)) == -1)
              break;
            setst(sx, GO);
            if ((sx = MN_spawn(mn[i].o.x + 15, mn[i].o.y, 1, MN_SOUL)) == -1)
              break;
            setst(sx, GO);
            if ((sx = MN_spawn(mn[i].o.x, mn[i].o.y - 10, 1, MN_SOUL)) == -1)
              break;
            setst(sx, GO);
          }
          break;
      ...
      case RUNOUT:
        if (t == MN_CYBER || t == MN_SPIDER || t == MN_BSP) {
          if (mn[i].ac == 0 || mn[i].ac == 6)
            Z_sound(snd[t - 1][2], 128);
        } else if (t == MN_ROBO)
          if (mn[i].ac == 0 || mn[i].ac == 12)
            Z_sound(snd[t - 1][2], 128);
      }
    }
}
...
void MN_draw(void) {
  ...
  for (i = 0; i < MAXMN; ++i)
    if (mn[i].t) {
      if (mn[i].t >= MN_PL_DEAD) {
        Z_drawmanspr(mn[i].o.x, mn[i].o.y, pl_spr[mn[i].t - MN_PL_DEAD], 0,
                     mn[i].d);
        continue;
      }
      if ((mn[i].t != MN_SOUL && mn[i].t != MN_PAIN) || mn[i].st != DEAD) {
        if (mn[i].t != MN_MAN)
          ...
          if (mn[i].ap[mn[i].ac] == 'E' || mn[i].ap[mn[i].ac] == 'F')
            Z_drawspr(mn[i].o.x, mn[i].o.y, sgun[mn[i].ap[mn[i].ac] - 'E'],
                      mn[i].d);
          Z_drawmanspr(
              mn[i].o.x, mn[i].o.y,
              spr[mn[i].t - 1][(mn[i].ap[mn[i].ac] - 'A') * 2 + mn[i].d],
...
int MN_hit(int n, int d, int o, int t) {
  ...
  if (mn[n].st == DEAD || mn[n].st == DIE)
    return 0;
  if (o == n) {
    if (t != HIT_ROCKET && t != HIT_ELECTRO)
      return 0;
    if (mn[n].t == MN_CYBER || mn[n].t == MN_BARREL)
      return 1;
  }
  if (o >= 0) {
    if (mn[o].t == MN_SOUL && mn[n].t == MN_PAIN)
      return 0;
    if (mn[o].t == mn[n].t)
      switch (mn[n].t) {
      case MN_IMP:
...
#define hit(o, x, y)                                                           \
  (y <= o.y && y > o.y - o.h && x >= o.x - o.r && x <= o.x + o.r)
...
int Z_hit(obj_t *o, int d, int own, int t) {
  ...
  hit_xv = o->xv + o->vx;
  hit_yv = o->yv + o->vy;
  if (Z_overlap(o, &pl1.o))
    if (PL_hit(&pl1, d, own, t)) {
  ...
  if (_2pl)
    if (Z_overlap(o, &pl2.o))
      if (PL_hit(&pl2, d, own, t)) {
...
int Z_hitobj(int obj, int d, int own, int t) {
  hit_xv = hit_yv = 0;
  if (obj == -1)
    return PL_hit(&pl1, d, own, t);
  else if (obj == -2 && _2pl)
    return PL_hit(&pl2, d, own, t);
  else if (obj < 0 || obj >= MAXMN)
    return 0;
  if (mn[obj].t)
    return MN_hit(obj, d, own, t);
  return 0;
}
...
void Z_explode(int x, int y, int rad, int own) {
  ...
  if (x < -100 || x > FLDW * CELW + 100)
    return;
  if (y < -100 || y > FLDH * CELH + 100)
    return;
  r = (long)rad * rad;
  dx = pl1.o.x - x;
  dy = pl1.o.y - pl1.o.h / 2 - y;
...
int Z_chktrap(int t, int d, int o, int ht) {
  ...
  if (_2pl)
    if (Z_istrapped(pl2.o.x, pl2.o.y, pl2.o.r, pl2.o.h)) {
      s = 1;
      if (t)
        PL_hit(&pl2, d, o, ht);
    }
...
void MN_warning(int l, int t, int r, int b) {
  ...
  for (i = 0; i < MAXMN; ++i)
    if (mn[i].t && mn[i].t != MN_CACO && mn[i].t != MN_SOUL &&
