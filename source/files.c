...
typedef struct { byte n, i, v, d; } dmv;
...
char f_drive[__MAX_DRIVE], f_dir[__MAX_DIR], f_name[__MAX_FNAME],
    f_ext[__MAX_EXT], f_path[__MAX_PATH];

void F_startup(void) {
  logo("F_startup: init FS\n");
...
void F_getsavnames(void) {
  ...
  for (i = 0; i < 7; ++i) {
    ...
    savok[i] = 0;
    if ((h = fopen(p, "rb")) == NULL)
      continue; // if((h=open(n,O_RDONLY|O_BINARY))==-1) continue;
    myfread(savname[i], 1, 24, h);
    ver = -1;
    myfread(&ver, 1, 2, h);
    fclose(h);
    savname[i][23] = 0;
    savok[i] = (ver == 3) ? 1 : 0; // savok[i]=(ver==2)?1:0;
  }
}
...
void F_savegame(int n, char *s) {
  ...
  char *p;
  p = getsavfpname(n, 0);
  if ((h = fopen(p, "wb")) == NULL)
    return;
  myfwrite(s, 1, 24, h);
  myfwrite("\3\0", 1, 2, h); // myfwrite("\2\0",1,2,h);
  G_savegame(h);
  W_savegame(h);
  DOT_savegame(h);
  SMK_savegame(h);
...
void F_addwad(char *fn) {
  ...
  ERR_failinit("can't add WAD %s", fn);
}
...
extern void mysplitpath(const char *path, char *drv, char *dir, char *name,
                        char *ext);
...
void F_initwads(void) {
  ...
  logo("F_initwads: loading WAD files\n");
  for (i = 0; i < MAX_WAD; ++i)
    wad[i].n[0] = 0;
  logo("   loading  %s\n", wads[0]);
  if ((wadh[0] = h = fopen(wads[0], "rb")) ==
      NULL) // if((wadh[0]=h=open(wads[0],O_RDWR|O_BINARY))==-1)
    ERR_failinit("can't open WAD %s", wads[0]); // sys_errlist[errno]);
  *s = 0;
  myfread(s, 1, 4, h);
  if (strncmp(s, "IWAD", 4) != 0 && strncmp(s, "PWAD", 4) != 0)
    ERR_failinit("invalid WAD signature");
  myfread(&n, 1, 4, h);
  myfread(&o, 1, 4, h);
  fseek(h, o, SEEK_SET);
  for (j = 0, p = 0; j < n; ++j) {
  ...
  for (i = 1; i < MAX_WADS; ++i)
    if (wads[i][0] != 0) {
      logo("  loading %s\n", wads[i]);
      if ((wadh[i] = h = fopen(wads[i], "rb")) ==
          NULL) // if((wadh[i]=h=open(wads[i],O_RDONLY|O_BINARY))==-1)
        ERR_failinit("can't open WAD %s", wads[i]); // sys_errlist[errno]);
...
void F_loadres(int r, void *p, dword o, dword l) {
  ...
  if (fseek(fh, wad[r].o + o, SEEK_SET) != 0)
    ERR_fatal("error reading file");

  if ((dword)myfreadc(p, 1, l, fh) != l)
    ERR_fatal("error loading resource %.8s", wad[r].n);
...
int F_getresid(char *n) {
  ...
  for (i = 0; i < wad_num; ++i)
    if (strncasecmp(wad[i].n, n, 8) == 0)
      return i;
  ERR_fatal("F_getresid: can't find resource %.8s", n);
  return -1;
}
...
void F_getresname(char *n, int r) { memcpy(n, wad[r].n, 8); }
...
int F_getsprid(char n[4], int s, int d) {
  ...
  for (i = s_start + 1; i < s_end; ++i)
    if (strncasecmp(wad[i].n, n, 4) == 0 &&
        (wad[i].n[4] == s || wad[i].n[6] == s)) {
      if (wad[i].n[4] == s)
        a = wad[i].n[5];
      else
        a = 0;
  ...
  ERR_fatal("F_getsprid: can't load sprite %.4s%c%c", n, (byte)s, (byte)d);
  return -1;
}

int F_getreslen(int r) { return wad[r].l; }
...
void F_loadmap(char n[8]) {
  ...
  if (memcmp(hdr.id, "Doom2D\x1A", 8) != 0)
    ERR_fatal("%.8s is not a valid map", n);
  for (;;) {
    ...
    if (!G_load(h))
      if (!W_load(h))
        if (!IT_load(h))
          if (!SW_load(h))
            ERR_fatal("illegal block %d(%d) in map %.8s", blk.t, blk.st, n);
    fseek(h, o, SEEK_SET);
  }
}
