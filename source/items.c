...
void IT_alloc(void) {
  int i, j, n;
  static char nm[][6] = {"ITEMUP", "WPNUP", "GETPOW", "ITMBK"},
              snm[][4] = {"CLIP", "SHEL", "ROCK", "CELL", "AMMO", "SBOX",
                          "BROK", "CELP", "STIM", "MEDI", "BPAK", "CSAW",
                          "SHOT", "SGN2", "MGUN", "LAUN", "PLAS", "BFUG"},
              n4[][4] = {"SOUL", "SMRT", "SMGT", "SMBT"},
              n3[][4] = {"GOR1", "FCAN"};
...
int IT_load(FILE *h) {
  ...
  case MB_THING:
    ...
    if (!g_dm) {
      if (j == -1)
        ERR_fatal("player 1 start not found");
      dm_pos[0].x = it[j].o.x;
      dm_pos[0].y = it[j].o.y;
    ...
    if (!g_dm && _2pl) {
      if (j == -1)
        ERR_fatal("player 2 start not found");
      dm_pos[1].x = it[j].o.x;
      dm_pos[1].y = it[j].o.y;
    ...
    if (g_dm && j < 2)
      ERR_fatal("less than 2 DM starts");
    if (g_dm) {
      dm_pnum = j;
    ...
    for (i = 0; i < m; ++i)
      if (it[i].t >= TH_CLIP && it[i].t < TH_DEMON) {
        ...
        MN_spawn(it[i].o.x, it[i].o.y, it[i].s & THF_DIR,
                 it[i].t - TH_DEMON + MN_DEMON);
        it[i].t = 0;
      }
    return 1;
  }
