...
int wp_it[11] = {0,      I_CSAW, 0,     I_SGUN, I_SGUN2, I_MGUN,
                 I_LAUN, I_PLAS, I_BFG, I_GUN2, 0};
...
static byte goanim[] = "BDACDA", dieanim[] = "HHHHIIIIJJJJKKKKLLLLMMMM",
...
void PL_savegame(FILE *h) {
  myfwrite(&pl1, 1, sizeof(pl1) - sizeof(_keys),
           h); // myfwrite(&pl1,1,sizeof(pl1),h);
  if (_2pl)
    myfwrite(&pl2, 1, sizeof(pl2) - sizeof(_keys),
             h); // myfwrite(&pl2,1,sizeof(pl2),h);
  myfwrite(&PL_JUMP, 1, 4, h);
  myfwrite(&PL_RUN, 1, 4, h);
  myfwrite(&p_immortal, 1, 1, h);
}

void PL_loadgame(FILE *h) {
  myfread(&pl1, 1, sizeof(pl1) - sizeof(_keys),
          h); // myfread(&pl1,1,sizeof(pl1),h);
  if (_2pl)
    myfread(&pl2, 1, sizeof(pl2) - sizeof(_keys),
            h); // myfread(&pl2,1,sizeof(pl2),h);
  myfread(&PL_JUMP, 1, 4, h);
  myfread(&PL_RUN, 1, 4, h);
  myfread(&p_immortal, 1, 1, h);
}
...
static void fire(player_t *p) {
  static fire_f *ff[11] = {WP_pistol,   WP_pistol,  WP_pistol, WP_shotgun,
                           WP_dshotgun, WP_mgun,    WP_rocket, WP_plasma,
                           WP_bfgshot,  WP_shotgun, WP_pistol};
  ...
  if (keys[p->kf] || p->wpn == 8) {
    ...
    if (p->wpn == 10)
      WP_ognemet(p->o.x, p->o.y - 15, p->o.x + ((p->d) ? 30 : -30),
                 p->o.y - 15 + firediry(p), p->o.xv + p->o.vx,
                 p->o.yv + p->o.vy, p->id);
    else if (p->wpn >= 1)
      ff[p->wpn](p->o.x, p->o.y - 15, p->o.x + ((p->d) ? 30 : -30),
...
static void jump(player_t *p, int st) {
  ...
  if (keys[p->kj]) {
    if (p_fly) {
      ...
      if (Z_canstand(p->o.x, p->o.y, p->o.r))
        p->o.yv = -PL_JUMP;
      else if (st & Z_INWATER)
        p->o.yv = -PL_SWUP;
    }
  }
}
...
void PL_alloc(void) {
  int i, j;
  static char nm[][6] = {"OOF",   "PLPAIN", "PLDETH", "SLOP",   "PDIEHI", "BFG",
                         "SAWUP", "SAWIDL", "SAWFUL", "SAWHIT", "PLFALL"};
  static char s[6];
...
int PL_hit(player_t *p, int d, int o, int t) {
  ...
  if (t != HIT_WATER && t != HIT_ELECTRO)
    ...
    FX_bubble(p->o.x, p->o.y - 20, 0, 0, d / 2);
  if (p_immortal || p->invl)
    return 1;
  p->hit += d;
  p->hito = o;
  return 1;
...
void PL_damage(player_t *p) {
  ...
  if ((p->life -= i) <= 0) {
    ...
    if (p->amul > 1)
      IT_spawn(p->o.x, p->o.y, I_BPACK);
    if (!g_dm) {
      if (p->keys & 16)
        IT_spawn(p->o.x, p->o.y, I_KEYR);
      if (p->keys & 32)
        IT_spawn(p->o.x, p->o.y, I_KEYG);
      if (p->keys & 64)
        IT_spawn(p->o.x, p->o.y, I_KEYB);
    }
    for (i = 1, p->wpns >>= 1; i < 11; ++i, p->wpns >>= 1)
      if (i != 2)
        if (p->wpns & 1)
          IT_spawn(p->o.x, p->o.y, wp_it[i]);
    p->wpns = 5;
    p->wpn = 2;
    p->f |= PLF_PNSND;
    p->drawst |= PL_DRAWWPN;
...
int PL_give(player_t *p, int t) {
  ...
  case I_MEDI:
    if (p->life >= 100)
      return 0;
    if ((p->life += ((t == I_MEDI) ? 25 : 10)) > 100)
      p->life = 100;
    p->drawst |= PL_DRAWLIFE;
    return 1;
  case I_CLIP:
    if (p->ammo >= 200 * p->amul)
      return 0;
    if ((p->ammo += 10) > 200 * p->amul)
      p->ammo = 200 * p->amul;
    p->drawst |= PL_DRAWWPN;
    return 1;
  case I_AMMO:
  ...
  case I_RBOX:
    if (p->rock >= 50 * p->amul)
      return 0;
    if ((p->rock += 5) > 50 * p->amul)
      p->rock = 50 * p->amul;
    p->drawst |= PL_DRAWWPN;
    return 1;
  case I_CELL:
    if (p->cell >= 300 * p->amul)
      return 0;
    if ((p->cell += 40) > 300 * p->amul)
      p->cell = 300 * p->amul;
    p->drawst |= PL_DRAWWPN;
    return 1;
  case I_CELP:
...
void PL_act(player_t *p) {
  ...
  if (keys[p->kp])
    SW_press(p->o.x, p->o.y, p->o.r, p->o.h, 1 | p->keys, p->id);
  if (p->fire)
    --p->fire;
  if (p->cwpn)
    --p->cwpn;
  ...
  case GO:
    ...
    jump(p, st);
    if (p_fly)
      SMK_gas(p->o.x, p->o.y - 2, 2, 3, p->o.xv + p->o.vx, p->o.yv + p->o.vy,
              128);
    if ((p->s += abs(p->o.xv) / 2) >= 24)
      p->s %= 24;
    ...
    if (p->o.xv < PL_RUN && keys[p->kr]) {
      p->o.xv += PL_RUN >> 3;
      p->d = 1;
    } else if (PL_RUN > 8)
      SMK_gas(p->o.x, p->o.y - 2, 2, 3, p->o.xv + p->o.vx, p->o.yv + p->o.vy,
              32);
    if (p->o.xv > -PL_RUN && keys[p->kl]) {
      p->o.xv -= PL_RUN >> 3;
      p->d = 0;
    } else if (PL_RUN > 8)
      SMK_gas(p->o.x, p->o.y - 2, 2, 3, p->o.xv + p->o.vx, p->o.yv + p->o.vy,
  ...
  case OUT:
    ...
    if (keys[p->ku] || keys[p->kd] || keys[p->kl] || keys[p->kr] ||
        keys[p->kf] || keys[p->kj] || keys[p->kp] || keys[p->kwl] ||
        keys[p->kwr]) {
      if (p->st != OUT)
        MN_spawn_deadpl(&p->o, p->color, (p->st == MESS) ? 1 : 0);
      PL_restore(p);
      if (g_dm) {
        G_respawn_player(p);
        break;
...
void PL_draw(player_t *p) {
  ...
  if (p->wpn == 0)
    w = 0;
  if (w)
    Z_drawspr(p->o.x + wx, p->o.y + wy, wpn[p->wpn][w - 'A'], p->d);
  if (s)
    Z_drawmanspr(p->o.x, p->o.y, spr[(s - 'A') * 2 + p->d],
                 sprd[(s - 'A') * 2 + p->d], p->color);
}

void *PL_getspr(int s, int d) { return spr[(s - 'A') * 2 + d]; }
...
void bfg_fly(int x, int y, int o) {
  ...
  if (_2pl)
    if (o != -2)
      chk_bfg(&pl2, x, y);
  if (o == -1 || o == -2)
    MN_warning(x - 50, y - 50, x + 50, y + 50);
}
