...
#define PCOLORN 10
byte pcolortab[PCOLORN] = {0x18, 0x20, 0x40, 0x58, 0x60,
                           0x70, 0x80, 0xB0, 0xC0, 0xD0};
...
static char *main_txt[] = {"NEW GAME", "LOAD GAME", "SAVE GAME", "OPTIONS",
                           "EXIT"},
            *opt_txt[] = {"RESTART", "VOLUME", "BRIGHTNESS", "MUSIC",
                          "FULLSCREEN:"},
            *ngplr_txt[] = {"ONE PLAYER", "TWO PLAYERS"},
            *ngdm_txt[] = {"COOPERATIVE", "DEATHMATCH"},
            *vol_txt[] = {"SOUND", "MUSIC"},
            *plcolor_txt[] = {"FIRST", "SECOND"}, *gamma_txt[] = {""};

static byte main_typ[] = {NEWGAME, LOADGAME, SAVEGAME, OPTIONS, QUITGAME},
            ngplr_typ[] = {PLR1, PLR2}, ngdm_typ[] = {COOP, DM},
            opt_typ[] = {ENDGAME, VOLUME, GAMMA, MUSIC, INTERP},
            quit_typ[] = {QUIT, CANCEL}, endgm_typ[] = {ENDGM, CANCEL},
            vol_typ[] = {SVOLM, MVOLM}, plcolor_typ[] = {PL1CM, PL2CM},
            gamma_typ[] = {GAMMAM},
            load_typ[] = {LOAD, LOAD, LOAD, LOAD, LOAD, LOAD, LOAD},
            save_typ[] = {SAVE, SAVE, SAVE, SAVE, SAVE, SAVE, SAVE};

static menu_t main_mnu = {MENU, 5, 0, 80, "MENU", main_txt, main_typ},
              opt_mnu = {MENU, 5, 0, 75, "OPTIONS", opt_txt, opt_typ},
              ngplr_mnu = {MENU, 2, 0, 90, "NEW GAME", ngplr_txt, ngplr_typ},
              ngdm_mnu = {MENU, 2, 0, 90, "GAME TYPE", ngdm_txt, ngdm_typ},
              vol_mnu = {MENU, 2, 0, 40, "VOLUME", vol_txt, vol_typ},
              plcolor_mnu = {MENU, 2, 0, 90, "COLOR", plcolor_txt, plcolor_typ},
              gamma_mnu = {MENU, 1, 0, 85, "BRIGHTNESS", gamma_txt, gamma_typ},
              load_mnu = {MENU, 7, 0, 85, "LOAD GAME", NULL, load_typ},
              save_mnu = {MENU, 7, 0, 85, "SAVE GAME", NULL, save_typ},
              quit1_msg = {MSG, 0, 0, 0, "ARE YOU SURE?", NULL, quit_typ},
              quit2_msg = {MSG, 0, 0, 0, "ARE YOU SURE?", NULL, quit_typ},
              quit3_msg = {MSG, 0, 0, 0, "ARE YOU SURE?", NULL, quit_typ},
              endgm_msg = {MSG, 0, 0, 0, "RESTART LEVEL?", NULL, endgm_typ};
...
static vgaimg *msklh[2], *mbarl, *mbarm, *mbarr, *mbaro, *mslotl, *mslotm,
    *mslotr;
static byte cbuf[32];
...
void G_code(void) {
  ...
  if (memcmp(cbuf + 32 - 5, "IDDQD", 5) == 0) {
    ...
    if (cbuf[30] >= '0' && cbuf[30] <= '9' && cbuf[31] >= '0' &&
        cbuf[31] <= '9') {
...
void GM_set(menu_t *m) {
  ...
  if (g_st == GS_GAME) {
    // V_setrect(0,SCRW,0,SCRH);V_clr(0,SCRW,0,SCRH,0);//V_setrect(0,320,0,200);V_clr(0,320,0,200,0);
     //if(_2pl)
    // {V_setrect(SCRW-120,120,0,SCRH);w_o=0;Z_clrst();w_o=SCRH/2;Z_clrst();}//if(_2pl)
    // {V_setrect(200,120,0,200);w_o=0;Z_clrst();w_o=100;Z_clrst();} else
    // {V_setrect(SCRW-120,120,0,SCRH);w_o=0;Z_clrst();}//else
    // {V_setrect(200,120,50,100);w_o=50;Z_clrst();}
...
void GM_command(int c) {
  ...
  case QUIT:
    ...
    GMV_stop();
#ifndef DEMO
    c = Z_sound(M_lock(qsnd[myrand(QSND_NUM)]),256);
    S_wait();
#endif
    ERR_quit();
    break;
...
byte get_keychar(int keysym) { return 0; }
...
int GM_act(void) {
  ...
  if (input)
    ...
    default:
      if (icur >= 23)
        break;
      c = 'X'; // get_keychar(lastkey);//c=keychar[(keys[0x2A] ||
               // keys[0x36])?1:0][lastkey];
      if (!c)
        break;
      ibuf[icur] = c;
      ibuf[++icur] = 0;
    ...
    case VK_START: // case 1:
      ...
      break;
    case VK_DUP: // case 0x48: case 0xC8:
      if (!mnu)
        break;
      if (mnu->type != MENU)
        break;
      ...
      Z_sound(msnd1, 128);
      break;
    case VK_DDOWN: // case 0x50: case 0xD0: case 0x4C:
      if (!mnu)
        break;
      if (mnu->type != MENU)
        break;
      ...
      break;
    case VK_DLEFT:
    case VK_DRIGHT: // case 0x4B: case 0x4D: case 0xCB: case 0xCD:
      if (!mnu)
        break;
      if (mnu->type != MENU)
        break;
      ...
      GM_command(mnu->t[mnu->cur] +
                 ((lastkey == VK_DLEFT)
                      ? 0
                      : 1)); // GM_command(mnu->t[mnu->cur]+((lastkey==0x4B ||
                             // lastkey==0xCB)?0:1));
      GM_set(mnu);
      if (!movsndt)
        movsndt = Z_sound((lastkey == VK_DLEFT) ? msnd5 : msnd6,
                          255); // if(!movsndt) movsndt=Z_sound((lastkey==0x4B
                                // || lastkey==0xCB)?msnd5:msnd6,255);
      break;
    case VK_A:
    case VK_X: // case 0x1C: case 0x39: case 0x9C:
      if (!mnu)
        break;
      if (mnu->type != MENU)
        break;
...
void GM_init(void) {
  ...
  static char nm[QSND_NUM][6] = {
      "CYBSIT", "KNTDTH", "MNPAIN", "PEPAIN", "SLOP",   "MANSIT", "BOSPN",
      "VILACT", "PLFALL", "BGACT",  "BGDTH2", "POPAIN", "SGTATK", "VILDTH"};
  char s[8];
  ...
  if (mnu->type == MENU) {
    ...
    for (i = 0; i < mnu->n; ++i) {
      if (mnu->t[i] == LOAD || mnu->t[i] == SAVE) {
        ...
        Z_gotoxy(
            mnu->x +
                ((mnu->t[i] >= SVOLM) ? ((mnu->t[i] >= PL1CM) ? 50 : 152) : 0),
            y + i * 16 + 20);
        Z_printbf(mnu->m[i]);
      }
      if (mnu->t[i] == MUSIC) {
        ...
        V_manspr(mnu->x + ((mnu->t[i] == PL1CM) ? 15 : 35),
                 y + i * 16 + 20 + 14, PL_getspr(*panimp, 0),
