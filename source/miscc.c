...
static void *sth[22], *bfh[160 - '!'], *sfh[160 - '!'], *bulsnd[2], *stone,
    *stone2, *keys[3];
...
void Z_initst(void) {
  ...
  static char nm[22][8] = {
      ...
      "STTNUM6", "STTNUM7", "STTNUM8", "STTNUM9", "STTMINUS", "STTPRCNT",
  ...
  for (i = '!'; i < 160; ++i) {
    ...
    if (!(i & 15))
      logo_gas(GAS_START + ((i - '!') >> 4), GAS_TOTAL);
  }
  for (i = '!'; i < 160; ++i) {
    sprintf(s, "STCFN%03d", i);
    sfh[i - '!'] = M_lock(F_findres(s));
    if (!(i & 15))
      logo_gas(GAS_START + 8 + ((i - '!') >> 4), GAS_TOTAL);
  }
  strcpy(s, "WINUM*");
  for (i = '0'; i <= '9'; ++i) {
    s[5] = i;
...
void Z_drawspr(int x, int y, void *p, char d) {
  if (d)
    V_spr2(x - w_x + WD / 2, y - w_y + HT / 2 + 1 + w_o,
           p); // if(d) V_spr2(x-w_x+100,y-w_y+HT/2+1+w_o,p);
  else
    V_spr(x - w_x + WD / 2, y - w_y + HT / 2 + 1 + w_o,
          p); // else V_spr(x-w_x+100,y-w_y+HT/2+1+w_o,p);
}
...
void Z_drawstlives(char n) {
  ...
  Z_clrst();                          // V_setrect(280,30,w_o,40);Z_clrst();
...
void Z_drawstkeys(byte k) {
  ...
  for (k >>= 4, n = 0, x = SCRW - 75; n < 3;
       ++n, k >>= 1, x += 9) // for(k>>=4,n=0,x=245;n<3;++n,k>>=1,x+=9)
    if (k & 1)
...
void Z_drawstnum(int n) {
  ...
  sprintf(s, "%d", n);
  l = strlen(s);
  x = (115 - l * 14) + SCRW - 120; // l=strlen(s);x=(115-l*14)+200;
  for (i = 0; i < l; ++i, x += 14) {
    if (s[i] >= '0' && s[i] <= '9')
      c = s[i] - '0';
    else if (s[i] == '-')
...
void Z_drawstwpn(int n, int a) {
  ...
  if (i >= 0)
    V_spr(SCRW - 88, w_o + 58 + 19,
          sth[i + 12]); // if(i>=0) V_spr(232,w_o+58+19,sth[i+12]);
  if (n >= 2) {
    sprintf(s, "%d", a);
    l = strlen(s);
...
void Z_drawmanspr(int x, int y, void *p, char d, byte color) {
  if (d)
    V_manspr2(x - w_x + WD / 2, y - w_y + HT / 2 + 1 + w_o, p,
              color); // if(d) V_manspr2(x-w_x+100,y-w_y+HT/2+1+w_o,p,color);
  else
    V_manspr(x - w_x + WD / 2, y - w_y + HT / 2 + 1 + w_o, p,
             color); // else V_manspr(x-w_x+100,y-w_y+HT/2+1+w_o,p,color);
}
