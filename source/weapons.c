...
void WP_loadgame(FILE *h) {
  ...
  myfread(&n, 1, 4, h);
  myfread(wp, 1, n * sizeof(wp[0]), h);
}

void WP_alloc(void) {
  int i;
  static char nm[14][6] = {"PISTOL", "SHOTGN", "DSHTGN", "RLAUNC", "RXPLOD",
                           "PLASMA", "FIRSHT", "FIRXPL", "BAREXP", "PUNCH",
                           "SAWHIT", "MGUN",   "SPARK1", "SPARK2"};
  ...
  for (i = 0; i < MAXWPN; ++i)
    if (wp[i].t) {
      if (wp[i].t == ROCKET || wp[i].t == REVF)
        SMK_gas(wp[i].o.x + Z_sign(wp[i].o.xv) * 2, wp[i].o.y - wp[i].o.h / 2,
                3, 3, wp[i].o.xv + wp[i].o.vx, wp[i].o.yv + wp[i].o.vy, 64);
      --wp[i].o.yv;
      st = Z_moveobj(&wp[i].o);
      if (st & Z_FALLOUT) {
        wp[i].t = 0;
      ...
      if (st & Z_HITWATER)
        ...
        case BFGBALL:
          break;
        default:
          Z_splash(&wp[i].o, wp[i].o.r + wp[i].o.h);
          break;
        }
      switch (wp[i].t) {
      case REVF:
        if (Z_getobjpos(wp[i].target, &o))
          throw(i, wp[i].o.x, wp[i].o.y - 2, o.x + o.xv + o.vx,
                o.y + o.yv + o.vy, 2, 5, 12);
      case ROCKET:
        if (wp[i].s >= 2) {
          if (++wp[i].s >= 8)
            wp[i].t = 0;
      ...
      case MANF:
        if (wp[i].s >= 2) {
          if (++wp[i].s >= ((wp[i].t == BALL1 || wp[i].t == BALL7 ||
                             wp[i].t == BALL2 || wp[i].t == MANF)
                                ? 8
                                : 12))
            wp[i].t = 0;
          break;
        ...
        if (st & Z_HITAIR)
          Z_set_speed(&wp[i].o, 16);
        if (st & (Z_HITWALL | Z_HITCEIL | Z_HITLAND)) {
          wp[i].s = 2;
          wp[i].o.xv = wp[i].o.yv = 0;
          Z_sound(snd[7], 128);
          break;
        } else if (Z_hit(&wp[i].o,
                         (wp[i].t == BALL7 || wp[i].t == MANF)
                             ? 40
                             : ((wp[i].t == BALL2) ? 20 : 5),
                         wp[i].own, HIT_SOME)) {
          wp[i].s = 2;
    ...
    if (((m = bmap[y >> 5][x >> 5]) & f)) {
      if (m & BM_WALL)
        if (fld[y >> 3][x >> 3] == 1 || fld[y >> 3][x >> 3] == 2) {
          for (x = lx, y = ly, xe = ye = 0;
               fld[y >> 3][x >> 3] != 1 && fld[y >> 3][x >> 3] != 2;) {
            lx = x;
            ly = y;
            if ((xe += xd) >= d) {
              xe -= d;
      ...
      if (m & (BM_MONSTER | BM_PLR1 | BM_PLR2))
        if (Z_gunhit(x, y, o, sx * v, sy * v))
          break;
      lx = x;
      ly = y;
      if ((xe += (xd << 3)) >= d) {
