...
byte flametab[16] = {0xBC, 0xBA, 0xB8, 0xB6, 0xB4, 0xB2, 0xB0, 0xD5,
  ...
  for (i = 0; i < MAXSMOK; ++i)
    if (sm[i].t)
      ...
      case 0:
        ...
        V_sprf(
            (sm[i].x >> 8) - w_x + WD / 2,
            (sm[i].y >> 8) - w_y + HT / 2 + 1 + w_o, spr[s],
            &smoke_sprf); // V_sprf((sm[i].x>>8)-w_x+100,(sm[i].y>>8)-w_y+50+w_o,spr[s],&smoke_sprf);
        break;
      case 1:
        if ((s = sm[i].t) >= (FLSN - 1))
          s = 0;
        ...
        V_sprf(
            (sm[i].x >> 8) - w_x + WD / 2,
            (sm[i].y >> 8) - w_y + HT / 2 + 1 + w_o, fspr[s],
            &flame_sprf); // V_sprf((sm[i].x>>8)-w_x+100,(sm[i].y>>8)-w_y+50+w_o,fspr[s],&flame_sprf);
        break;
      }
}
...
void SMK_flame(int x0, int y0, int ox, int oy, int xr, int yr, int xv, int yv,
               int k, int o) {
  int i, x, y;
  static int sxv, syv;
