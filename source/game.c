...
void G_savegame(FILE *h) {
  myfwrite(&_2pl, 1, 1, h);
  myfwrite(&g_dm, 1, 1, h);
  myfwrite(&g_exit, 1, 1, h);
  myfwrite(&g_map, 1, 1, h);
  myfwrite(&g_time, 1, 4, h);
  myfwrite(&dm_pl1p, 1, 4, h);
  myfwrite(&dm_pl2p, 1, 4, h);
  myfwrite(&dm_pnum, 1, 4, h);
  myfwrite(dm_pos, 1, dm_pnum * sizeof(pos_t), h);
...
void G_loadgame(FILE *h) {
  myfread(&_2pl, 1, 1, h);
  myfread(&g_dm, 1, 1, h);
  myfread(&g_exit, 1, 1, h);
  myfread(&g_map, 1, 1, h);
  myfread(&g_time, 1, 4, h);
  myfread(&dm_pl1p, 1, 4, h);
  myfread(&dm_pl2p, 1, 4, h);
  myfread(&dm_pnum, 1, 4, h);
  myfread(dm_pos, 1, dm_pnum * sizeof(pos_t), h);
  myfread(&cheat, 1, 1, h);
...
*/ g_trans = 0;
  ...
  if (g_st == GS_BVIDEO || g_st == GS_EVIDEO) {
    if (!A8_nextframe() || lastkey == VK_START) {
      if (lastkey == VK_START)
        lastkey = 0;
      A8_close();
      if (g_st == GS_BVIDEO)
  ...
  case GS_INTER:
#ifdef DEMO
    if (keys[VK_A] || keys[VK_B] || keys[VK_X]) {
      set_trans(GS_TITLE);
    }
#else
    if (keys[VK_A] || keys[VK_X]) // if(keys[0x39] || keys[0x1C] || keys[0x9C])
      if (!G_beg_video())
        G_start();
      else {
        g_st = GS_BVIDEO;
...
static void pl_info(player_t *p, int y) {
  ...
  Z_gotoxy(25, y + 30);
  Z_printbf("SECRETS %u / %u", p->secrets,
            sw_secrets); // Z_gotoxy(25,y+30);Z_printbf("SECRETS %u /
                         // %u",p->secrets,sw_secrets);
  Z_gotoxy(255, y);
  Z_printbf("%u", p->kills); // Z_gotoxy(255,y);Z_printbf("%u",p->kills);
  Z_gotoxy(255, y + 15);
  Z_printbf("%u.%u", t / 10,
            t % 10); // Z_gotoxy(255,y+15);Z_printbf("%u.%u",t/10,t%10);
}
...
void G_draw(void) {
  ...
  case GS_ENDSCR:
    ...
    V_pic(0, 0, scrnh[2]); // V_clr(0,320,0,200,0);V_pic(0,0,scrnh[2]);
    V_center(0);           //
    break;
  case GS_INTER:
    V_center(1);                //
  ...
  if (h)
    V_maptoscr(
        0, SCRW - 120, 1, (_2pl) ? (SCRH / 2 - 2) : (SCRH - 2),
        clrmap +
            h * 256); // if(h) V_maptoscr(0,200,(_2pl)?1:51,98,clrmap+h*256);
  else
    V_copytoscr(0, SCRW - 120, 1,
                (_2pl) ? (SCRH / 2 - 2)
                       : (SCRH - 2)); // else V_copytoscr(0,200,(_2pl)?1:51,98);
  if (pl1.drawst)
    V_copytoscr(
        SCRW - 120, 120, 0,
        _2pl ? (SCRH / 2)
             : SCRH); // if(pl1.drawst) V_copytoscr(200,120,(_2pl)?0:50,100);
  ...
  if (_2pl) {
    if (pl2.invl)
      ...
      h = 5;
    if (h)
      V_maptoscr(0, SCRW - 120, SCRH / 2 + 1, SCRH / 2 - 2,
                 clrmap +
                     h * 256); // if(h) V_maptoscr(0,200,101,98,clrmap+h*256);
    else
      V_copytoscr(0, SCRW - 120, SCRH / 2 + 1,
                  SCRH / 2 - 2); // else V_copytoscr(0,200,101,98);
    if (pl2.drawst)
      V_copytoscr(SCRW - 120, 120, SCRH / 2,
                  SCRH / 2); // if(pl2.drawst) V_copytoscr(200,120,100,100);
